    ...
    if key == "is-printer-installed" {
        ...
        if key.starts_with("download-data-") {
            ...
            return match (
                ...
                (Ok(true), false) => format!("RustWallpaper-{_version}-x86_64.msi"),
                (Ok(true), true) | (Ok(false), _) => format!("RustWallpaper-{_version}-x86_64.exe"),
                (Err(e), _) => {
            ...
            #[cfg(target_os = "macos")]
            {
                return if cfg!(target_arch = "x86_64") {
                    format!("RustWallpaper-{_version}-x86_64.dmg")
                } else if cfg!(target_arch = "aarch64") {
                    format!("RustWallpaper-{_version}-aarch64.dmg")
                } else {
